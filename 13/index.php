...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    $values = array();
    $values['name'] = $errors['name'] ? "" : strip_tags($_COOKIE['name_value']);
    $values['email'] = $errors['email'] ? "" : strip_tags($_COOKIE['email_value']);
...
// Иначе, если запрос был методом POST, т.е. нужно проверить данные и сохранить их в XML-файл.
    ...
    if (!empty($_COOKIE[session_name()]) && session_start() && !empty($_SESSION['login'])){
        ...
        $s = $xml->AsXML();
        $name = /*$_COOKIE['xml_cookie']*/$_SESSION['filename'];
        $xml->saveXML("$name");
    }
    else {
